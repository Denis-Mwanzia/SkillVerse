...
async function __jacCallFunction(function_name, args) {
  let token = __getLocalStorage("jac_token");
  let response = await fetch(`/function/${function_name}`, {"method": "POST", "headers": {"Content-Type": "application/json", "Authorization": (token ? `Bearer ${token}` : "")}, "body": JSON.stringify(args)});
  let payload = await response.json();
  if (!payload["ok"]) {
...
export { ErrorFallback, JacClientErrorBoundary, Link, Navigate, Route, Router, Routes, __jacCallFunction, __jacJsx, __jacSpawn, errorOverlay, jacIsLoggedIn, jacLogin, jacLogout, jacSignup, jacSpawn, navigate, useEffect, useLocation, useNavigate, useParams, useRouter, useState };
